 ...
 * ECE 2230, Fall 2016
 *
 * Purpose: This file contains drivers to test the mem.c package for dynamic memory
 * allocation.
 * 
 ...
 * -u 1      Tests 4 allocations including a new page and an exact fit.
 * -u 2      Tests all fit types and displays the differences. Also shows
 *           coalescing patters.
 *
 ...
 *
 *
 * Assumptions: A 64-bit machine is used to run the test. Although the tests 
 *              should work for a 32-bit machine exactly the same just with
 *              different sizes. 
 *
 *   
 *
...
int main(int argc, char **argv)
{
    ...
    else if (dprms.UnitDriver == 1)
    {
        ...
        //allocate a block of memory the size of PAGESIZE + 1
        num_bytes_4 = units_in_first_page*unit_size;
    ...
    /* This driver displays the differences between the search policies. It also
       displays coalescing patterns */    
    else if (dprms.UnitDriver == 2)
    {
        ...
        num_bytes_2 = (units_in_first_page/16 - 1)*unit_size;
        p2 = (int *) Mem_alloc(num_bytes_2);
        printf("second: %d bytes (%d units) p=%p \n", 
                num_bytes_2, num_bytes_2/unit_size, p2);
        Mem_print();
        ...
        /*allocate whole page --> notice that when coalescing, the blocks are combined
          and contain a whole page worth of memory --> won't need to malloc another page*/    
        ...
        printf("fourth free p=%p \n", p4);
        Mem_free(p4);
        Mem_print();

        printf("fifth free p=%p \n", p5);
        Mem_free(p5);
        Mem_print();

        num_bytes_6 = (units_in_first_page/8)*unit_size;
...
void equilibriumDriver(driver_params *ep)
{
    ...
    for (i = 0; i < ep->Trials; i++) {
        if (drand48() < 0.5) {
            ...
            list_insert(mem_list, (data_t *) ptr, LISTPOS_TAIL);
            ptr = NULL;
        } else if (list_entries(mem_list) > 0) {
            pos = (int) (drand48() * list_entries(mem_list));
    ...
    assert(list_entries(mem_list) == 0);
