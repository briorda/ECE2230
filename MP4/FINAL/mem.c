 ...
 * MP4
 *
 * Purpose: The file contains my implemenation of allocating and 
 *          freeing memory. It is for the lab4.c drivers to test
 *          efficiency of different search policies along with 
 *          the difference between my Mem_alloc and Mem_free 
 *          versus the standard library's version malloc and free.   
 *
 *
 * Assumptions: The free list ALWAYS contains at least the dummy
 *              block. Rover pointer points to a block of memory in 
 *              the free list. If the list is empty, the Rover
 *              should point to the Dummy Block. 
 *
 *   
 *
...
void Mem_free(void *return_ptr) {
    ...
    if (Coalescing && Rover->next != Rover) {
            ...
            do {
                prev = Rover;
                Rover = Rover->next; 
            } while ((free_block < prev || free_block > Rover) && Rover != &Dummy);\
            assert(Rover == &Dummy ? prev->next == &Dummy : prev < Rover);
          
            //coalesce with both neighbors
            if ((prev + prev->size == free_block) && 
                    (free_block + free_block->size == Rover)) {
                prev->size = prev->size + free_block->size + Rover->size;
                Rover = Rover->next;
                prev->next = Rover;
            }                        
            //coalesce with lower neighbor
            else if (prev + prev->size == free_block) 
                prev->size = prev->size + free_block->size;
            //coalesce with upper neighbor
            else if (free_block + free_block->size == Rover) {
                prev->next = free_block;
                free_block->size = free_block->size + Rover->size;
                Rover = Rover->next;
...
void Find_mem_block (const int nbytes, int fit_type, chunk_t **prev) {
    ...
    chunk_t *slow = Rover, *fast = Rover, *prev_mem_block = NULL, *mem_block = Rover;
    int nunits = nbytes / sizeof(chunk_t) + 1;
    int best_fit = 9999999, worst_fit = -1;
    ...
    if (fit_type == BEST_FIT) {
        while (slow && fast && fast->next) { 
            *prev = Rover;
            ...
            if (slow == fast || Rover->size-nunits == 0) {
                if (slow == fast) {
                    *prev = prev_mem_block;
                    Rover = mem_block;
                }
