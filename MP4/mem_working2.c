...
void *Mem_alloc(const int nbytes) {
    ...
    chunk_t *temp = Rover; //get current address of rover
    while (Rover->next->next != temp) {
    ...
    if (avail_block->next == temp) {
            ...
            assert(avail_block == p);
            assert((avail_block->size - 1)*sizeof(chunk_t) >= nbytes);
            assert(avail_block->next == NULL);  // saftey first!
    }
        
    //if NOT an exact fit -> split block in two
    if (avail_block->size > nunits) {

        int block_size = avail_block->size;
        split_block = avail_block + (block_size - nunits);
        avail_block = avail_block + nunits;
        split_block->size = block_size - nunits;
        avail_block->size = nunits;
        Mem_free(split_block+1);
        Rover = Rover->next;

        assert(avail_block->size + split_block->size == block_size);
        assert(avail_block->size == nbytes / sizeof(chunk_t));
    }
...
void Mem_stats(void) {
    ...
    printf("Min = %d    Max = %d    Average Size (B) = %d\n", min, max, avg);
