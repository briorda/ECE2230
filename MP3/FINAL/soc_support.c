 ...
 * MP3
 *
 * Purpose: The purpose of this file is to provide functions for the lab3.c file. The 
 *          reason it is separate from the main file is for information hiding and to 
 *          allow the programmer to use different data structures with the lab3.c file 
 *          without changing it.  
 *
...
list_t *soc_create(void) {
    return list_construct(soc_compare, soc_compare_qsort); 
}

int soc_compare_qsort(const void *p_a, const void *p_b) {
    return soc_compare(*(socket_t **) p_b, *(socket_t **) p_a);
}
...
/* Measures the performance of a sorting algorithm using the built in C function 
 * "clock" to count the number of cycles used by the program
 *
 *
 * list_ptr: pointer to list-of-interest 
 * sort_type: a number that corresponds to a sorting method...
 *      1 = insertion sort
 ...
 *
 */
void soc_sort (list_t *L, int sort_type) {

    int initialsize = list_entries(L);
    clock_t start, end;
    double elapse_time;
    ...
    assert(list_entries(L) == initialsize);
